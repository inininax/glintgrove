...
try {
  ...
  for (const master of baseline.masters) {
    await change(master.path, next => {
      const affected = next.assets.filter(a => a.master.path === master.path);
  ...
  try { const next = await run(); next.assets.filter(a => a.master.path === baseline.masters[0].path).forEach(review); }
