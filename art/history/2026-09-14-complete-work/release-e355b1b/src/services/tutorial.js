export function getTutorial(def, saveData) {
  ...
  if (def.id === 6 && !seen('tut-6')) {
    return { type: 'card', levelId: 6, art: 'splitter', titleKey: 'tutCardSplitterTitle', bodyKey: 'tutCardSplitterBody' };
  }
  if (def.id === 17 && !seen('tut-17')) {
    return { type: 'card', levelId: 17, art: 'crystal', titleKey: 'tutCardColorTitle', bodyKey: 'tutCardColorBody' };
  }
  if (def.id === 23 && !seen('tut-23')) {
    return { type: 'card', levelId: 23, art: 'portal', titleKey: 'tutCardPortalTitle', bodyKey: 'tutCardPortalBody' };
  }
  return null;
}
