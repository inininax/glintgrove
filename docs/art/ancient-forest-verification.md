...
- 사용자 `localhost:8000`의 기존 워커를 정상 업데이트한 뒤 네트워크 캐시와 서비스 워커 우회를 끄고 다시 로드했다. v8 코어 캐시와 현재 에셋 25개가 오류 없이 준비됐고, 타이틀 두 이미지도 디코딩됐다. 재로드 전후 `glintgrove_save_v2` 내용은 동일했다. 현재 화면은 `/tmp/ilyndrel-ancient-local-final.png`로 확인했다.

검증 캡처는 `/tmp/ilyndrel-ancient-title-desktop-v2.png`, `/tmp/ilyndrel-ancient-title-mobile-v2.png`, `/tmp/ilyndrel-ancient-title-short-v2.png`, `/tmp/ilyndrel-ancient-game-320.png`, `/tmp/ilyndrel-ancient-beam-first-contact.png`, `/tmp/ilyndrel-ancient-first-clear.png`, `/tmp/ilyndrel-ancient-guide-mobile-fixed.png`, `/tmp/ilyndrel-ancient-gate-guide.png`에 남겼다. 임시 캡처는 배포 파일이 아니다.
