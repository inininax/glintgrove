...
**플레이하기: [GitHub Pages에서 열기](https://inininax.github.io/glintgrove/)**

## v2.0 — Tetris Effect 톤 그래픽 리디자인 & 300 라운드

- **300 레벨 · 15 챕터**: 쉬움(32) / 보통(113) / 어려움(48) / 매우 어려움(107) 혼합 구성. 솔버 검증 파이프라인으로 전 레벨 풀이 가능성과 공정한 par 보장
- **블룸 포스트프로세스**: 쿼터 해상도 다운샘플 블러 → 합성. 화면 전체가 부드럽게 빛나는 PS5 Tetris Effect 톤
- **라이브 배경**: 네뷸라 그라디언트 + 시선을 끄는 십자 글린트 별 + 실시간 **오로라 리본**(각성 시 강해짐) + 3겹 숲 실루엣
- **이벤트 연출**:
  - 레벨 시작 — 발산원 **점화** 연출(입자 수렴 → 빔 reveal)
  - 각성 — 충격파 링 + **10방향 광선** + 잎사귀/포자 + 떠오르는 빛 입자 + 블룸 펄스
  - 클리어 — **슬로모션**(0.55초) → 타깃 순차 폭발 캐스케이드 → 오로라 폭발
- **수익화 요소 완전 제거**: 빛 스킨 4종 전부 무료 개방, 광고/과금 요소 없음

## v1.2 신기능

- **일일 챌린지 ☀️**: 날짜를 시드로 기존 레벨의 거울 배치를 절차적으로 재구성 — 매일 도전 가능한 새 퍼즐. 최적 이동수는 솔버가 실시간 계산해 공정한 별점을 보장합니다(연속 달성 스트릭 저장)
- **빛 스킨 4종**: 클래식 / 오션(★12) / 엠버(★30) / 오로라(★60) — 별 개수로 해금되는 코스메틱
- **인터랙티브 튜토리얼**: L1 첫 거울 가이드 포인터, L6/L17/L23 진입 시 신 메커니즘 설명 카드, 25초 무조작 시 힌트 제안
- **원격 설정 플래그**: 저장소 루트 `config.json` 수정만으로 par 조정·힌트 패널티·일일 난이도 범위 등을 재배포 없이 변경 (같은 origin fetch, CSP 안전)
- **영문 지원**: 설정에서 한국어/English 전환 (auto 감지)
- **업적 9종** + 진행 데이터 내보내기(JSON)

### 원격 설정 사용법

저장소 루트의 `config.json`을 수정 후 push하면 반영됩니다:
...
`parOverrides`로 특정 레벨의 목표 이동수만 조정할 수 있습니다 (예: 어려워서 이탈이 몰리는 레벨 완화).
